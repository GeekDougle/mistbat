...
data_file_path = XDG_DATA_HOME + "/mistbat/polkadot_csv_import.json"

def update_from_remote():
    """Look for files in XDG_DATA_HOME with the format "PolkadotStakingTransactions*.csv".  Look for 
    ...
    logger.debug(f'Reading Polkadot files from {XDG_DATA_HOME}')
    filenames = glob.glob(XDG_DATA_HOME+"/PolkadotStakingTransactions*.csv")
    logger.info(f'Found {len(filenames)} Polkadot files.')
    ...
    logger.info(f'Found {len(transactions)} Polkadot observations.')

    unique_transactions = []
    for d in transactions:
        if d not in unique_transactions:
            unique_transactions.append(d)
    logger.info(f'Found {len(unique_transactions)} Unique Polkadot observations.')
    ...
    for i, x in enumerate(op_hash_list):
        if x not in seen:
            ...
            if (prev_instance['Date'] != unique_transactions[i]['Date']) or prev_instance['Event Index'] != transactions[i]['Event Index']:
                warnings.warn('Non Matching Duplicate Polkadot Operation Hash: {}'.format(x))
            else:
                duplicate.add(i)
    ...
    if len(duplicate) >0:
        ...
        logger.warning(f'{len(duplicate)} duplicate transactions deleted.')
    ...
    #write the file out
    logger.info(f'Writing Polkadot Data to {data_file_path}...')
    with open(data_file_path, "w") as f:
        f.write(json.dumps(b_resources, indent=2))


def parse_events():
    """Take json file of Polkadot transactions and parse into Event instances.
    Returns:
      A list of instances of Event subclasses (e.g., Exchange, FiatExchange, Send)
      The location is "Polkadot"+ the text in the Account Name field.
    """
    ...
    for obs in json_data["deposits"]:
        receive = Receive(
            ...
            location="Polkadot",
            coin="DOT",
            amount=float(obs["Value"]),
            txid=f'{obs["Event Index"]}_{obs["Extrinsic Index"]}',
        )
    ...
    for obs in json_data["withdraws"]:
        send = Send(
            ...
            location="Polkadot" + '-' + obs['Account Name'],
